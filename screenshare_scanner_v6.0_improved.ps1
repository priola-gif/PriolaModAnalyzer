        ...
        foreach ($usb in $usb_drives) {
            if ($usb.DriveLetter) {
                ...
                $files = Get-ChildItem -Path $usb_path -File -Recurse -ErrorAction SilentlyContinue | 
                         Select-Object Name, FullName, LastWriteTime, Length | Select-Object -First 20
...
if ($usb_timeline.Count -gt 0) {
    ...
    foreach ($event in $usb_timeline | Select-Object -First 10) {
        Write-Log "  - $event"
...
if ($recycle_forensics.DeletedJARs.Count -gt 0) {
    ...
    foreach ($jar in $recycle_forensics.DeletedJARs | Select-Object -First 5) {
...
if ($usb_file_list.Count -gt 0) {
    ...
    foreach ($file in $usb_file_list | Select-Object -First 15) {
...
# [17/17] Summary
Write-Host "[17/17] Generating final verdict..." -ForegroundColor Yellow
Write-Log ""
Write-Log "═════════════════════════════════════════════════════════"
