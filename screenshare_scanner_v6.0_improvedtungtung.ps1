...
# ═══════════════════════════════════════════════════════════════
# DLL SCANNING FUNCTION
function Find-SuspiciousDLLs {
    param()
    
    $found_dlls = @()
    $suspicious_dll_names = @(
        "cheat", "hack", "mod", "inject", "loader", "bypass",
        "client", "bot", "auto", "crystal", "anchor", "totem",
        "vape", "nova", "meteor", "wurst", "impact", "grim",
        "hook", "patch", "wrapper", "proxy", "intercept"
    )
    ...
    try {
        ...
        if (Test-Path $minecraft_path) {
            ...
            foreach ($dll in $dlls) {
                ...
                foreach ($keyword in $suspicious_dll_names) {
                    if ($dll_name_lower -match $keyword) {
                        $found_dlls += @{
                            Filename = $dll.Name
                            Path = $dll.FullName
                            Size = $dll.Length
        ...
        # Check temp folder for injected DLLs
        $temp_dlls = Get-ChildItem -Path $env:TEMP -Filter "*.dll" -Recurse -ErrorAction SilentlyContinue -Force
        foreach ($dll in $temp_dlls) {
            $dll_name_lower = $dll.Name.ToLower()
...
# ═══════════════════════════════════════════════════════════════
# REGISTRY CHEAT CONFIG DETECTION
function Find-CheatRegistryConfig {
    ...
    try {
        ...
        $cheat_keywords = @(
            "cheat", "client", "hack", "mod", "crystal", "anchor",
            "vape", "nova", "meteor", "wurst", "impact", "grim"
        )
        ...
        foreach ($reg_path in $reg_paths) {
            try {
                ...
                foreach ($item in $items) {
                    ...
                    foreach ($keyword in $cheat_keywords) {
                        if ($item_name_lower -match $keyword) {
                            $cheat_configs += @{
                                Name = $item.PSChildName
                                Path = $item.PSPath
                                Type = "Registry Key"
                                Modified = $item.PSProvider
                            }
                            break
                        }
                    }
...
function Get-CheatNetworkConnections {
    ...
    $suspicious_ips = @()
    $cheat_domains = @(
        "cheatserver", "clientapi", "hack", "inject",
    ...
    try {
        ...
        foreach ($conn in $connections) {
            try {
                ...
                $hostname = [System.Net.Dns]::GetHostEntry($conn.RemoteAddress).HostName -ErrorAction SilentlyContinue
                
                if ($hostname) {
                    $hostname_lower = $hostname.ToLower()
                    
                    foreach ($domain in $cheat_domains) {
...
function Find-SuspiciousMacros {
    ...
    # Macro file extensions and patterns
    $macro_extensions = @("*.ahk", "*.lua", "*.vbs", "*.scr", "*.py", "*.js")
    $macro_keywords = @(
        "autohotkey", "autoclicker", "macro", "keybind", "keystroke",
        "mouseclick", "mousemove", "sendkeys", "keyboard", "input",
        "trigger", "hotkey", "script", "bot", "automation",
        "1.8 macro", "minecraft macro", "crystal", "clickbot", "1.8.9"
    )
    
    # Known macro software paths
    $macro_software_paths = @(
        ...
        "C:\Program Files (x86)\SteelSeries",
    ...
    try {
                ...
                foreach ($extension in $macro_extensions) {
                    try {
                        ...
                        foreach ($file in $files) {
                            # Skip system/legitimate scripts
                            if ($file.FullName -match "\\Windows\\|\\System32\\") {
                                continue
                            }
                            ...
                            try {
                                ...
                                # Check content for macro keywords
                                foreach ($keyword in $macro_keywords) {
        ...
        # Check for macro software installations
        foreach ($software_path in $macro_software_paths) {
            if (Test-Path $software_path) {
                $exe_files = Get-ChildItem -Path $software_path -Filter "*.exe" -ErrorAction SilentlyContinue
                
                if ($exe_files.Count -gt 0) {
                    $found_macros += @{
                        Filename = Split-Path $software_path -Leaf
                        Path = $software_path
                        Extension = ".exe (Software)"
                        Size = "Installation"
                        Created = (Get-Item $software_path).CreationTime
                        Modified = (Get-Item $software_path).LastWriteTime
