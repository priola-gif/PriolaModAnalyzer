# Priola Scanner 
# Integrated: BETA USBView + System Tools
...
Write-Host "║   PRIOLA SCANNER BETA - MOD PATH SETUP           ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
$MEOW_PATTERNS = @(
    ...
    "optimization", "C16d71e5042", "C8254494e5e",
...
function Find-SuspiciousDLLs {
    ...
    $known_cheat_dlls = @(
        ...
        # Paid cheats
        "prestige", "virgin", "skyclient", "lunar",
...
# ═══════════════════════════════════════════════════════════════
# POWERSHELL HISTORY - DETAILED COMMAND ANALYSIS
# ═══════════════════════════════════════════════════════════════

function Get-PowerShell-History-Deep {
    param()
    
    $findings = @()
    $history_file = "$env:USERPROFILE\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadline\ConsoleHost_history.txt"
    
    if (-not (Test-Path $history_file)) { return $findings }
    ...
    try {
        ...
        # Suspicious patterns
        $patterns = @(
            @{Pattern = "Invoke-WebRequest.*cheat|Invoke-WebRequest.*hack|Invoke-WebRequest.*nova|Invoke-WebRequest.*vape"; Type = "Cheat Download"},
            @{Pattern = "curl.*cheat|curl.*hack|curl.*nova|curl.*vape"; Type = "Cheat Download (curl)"},
            @{Pattern = "Move-Item.*USB|Move-Item.*`\w:`"; Type = "Moving files to USB/Drive"},
            @{Pattern = "Copy-Item.*USB|Copy-Item.*`\w:`"; Type = "Copying files to USB/Drive"},
            @{Pattern = "Remove-Item.*mods|Remove-Item.*\.minecraft"; Type = "Deleting Minecraft files"},
            @{Pattern = "Clear-RecycleBin|Empty-RecycleBin"; Type = "Clearing Recycle Bin"},
            @{Pattern = "Set-ExecutionPolicy.*Bypass"; Type = "Changed execution policy"},
            @{Pattern = "Start-Process.*powershell.*-Verb RunAs"; Type = "Elevated process"},
            @{Pattern = "robocopy.*mods|robocopy.*minecraft"; Type = "Copying Minecraft data"},
            @{Pattern = "wget.*download|aria2c"; Type = "Download tool usage"}
        )
        
        foreach ($cmd in $commands) {
            $cmd_lower = $cmd.ToLower()
            
            foreach ($pattern_obj in $patterns) {
                if ($cmd_lower -match $pattern_obj.Pattern) {
                    $findings += @{
                        Command = $cmd
                        Type = $pattern_obj.Type
                        Suspicious = $true
                    }
                    break
                }
...
# ═══════════════════════════════════════════════════════════════
# REGISTRY CHEAT CONFIGURATIONS
# ═══════════════════════════════════════════════════════════════

function Get-Registry-Cheat-Config {
    param()
    ...
    try {
        ...
        foreach ($reg_path in $reg_paths) {
            if (-not (Test-Path $reg_path)) { continue }
            
            try {
                $items = Get-ChildItem -Path $reg_path -ErrorAction SilentlyContinue -Force
                
                foreach ($item in $items) {
                    $name_lower = $item.PSChildName.ToLower()
                    
                    foreach ($cheat in $cheat_names) {
                        if ($name_lower -match [regex]::Escape($cheat)) {
                            # Get registry key with timestamps
                            $reg_item = Get-Item -Path $item.PSPath -Force
                            $last_modified = $reg_item.LastWriteTime
                            ...
                            $configs += @{
                                ...
                                LastModifiedFormatted = $last_modified.ToString("yyyy-MM-dd HH:mm:ss")
                                DaysOld = ((Get-Date) - $last_modified).Days
                            }
                            break
                        }
                    }
...
# ═══════════════════════════════════════════════════════════════
# LAUNCHER PROFILES & MOD LOADER ANALYSIS
# ═══════════════════════════════════════════════════════════════

function Get-Launcher-Profile-Analysis {
    param()
    ...
    try {
        $launcher_file = "$env:APPDATA\.minecraft\launcher_profiles.json"
        
        if (Test-Path $launcher_file) {
            $content = Get-Content $launcher_file -Raw | ConvertFrom-Json
            
            if ($content.profiles) {
                foreach ($profile_name in $content.profiles.PSObject.Properties.Name) {
                    $profile = $content.profiles.$profile_name
                    
                    $profile_info = @{
                        Name = $profile_name
                        Type = $profile.type
                        LastVersionId = $profile.lastVersionId
                        GameDirectory = $profile.gameDir
                    }
                    
                    # Check for Fabric
                    if ($profile.lastVersionId -match "fabric") {
                        $analysis.HasFabric = $true
                        $profile_info.Loader = "Fabric"
                    }
                    
                    # Check for Forge
                    if ($profile.lastVersionId -match "forge") {
                        $analysis.HasForge = $true
                        $profile_info.Loader = "Forge"
                    }
                    
                    # Check for suspicious JVM args
                    if ($profile.javaArgs) {
                        if ($profile.javaArgs -match "-jar.*cheat|-jar.*hack") {
                            $analysis.Issues += "Suspicious JVM arguments in $profile_name"
                            $profile_info.Suspicious = $true
                        }
                    }
                    
                    $analysis.Profiles += $profile_info
                }
            }
            
            # Check for multiple profiles (may indicate hidden cheat profile)
            if ($content.profiles.PSObject.Properties.Count -gt 5) {
                $analysis.Issues += "Unusual number of profiles ($(content.profiles.PSObject.Properties.Count))"
            }
        }
    } catch { }
...
# ═══════════════════════════════════════════════════════════════
Write-Host "╔════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║   PRIOLA SCANNER BETA - COMPLETE SUITE           ║" -ForegroundColor Cyan
Write-Host "║   WITH STRING DETECTION & DISPLAY                     ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════╝" -ForegroundColor Cyan

Write-Log "PRIOLA SCANNER BETA - $(Get-Date)"
Write-Log ""
...
# [26/29] Generating final verdict...
...
# [23/29] POWERSHELL HISTORY DEEP SCAN
Write-Host "[23/29] Scanning PowerShell command history..." -ForegroundColor Yellow
Write-Log "[23/29] POWERSHELL HISTORY ANALYSIS"

$ps_history = Get-PowerShell-History-Deep

if ($ps_history.Count -gt 0) {
    Write-Host "⚠ Suspicious PowerShell commands found:" -ForegroundColor Yellow
    
    foreach ($cmd_item in $ps_history) {
        Write-Host "  ⚠ [$($cmd_item.Type)]" -ForegroundColor Yellow
        Write-Host "    Command: $($cmd_item.Command)" -ForegroundColor Gray
        Write-Log "  ⚠ $($cmd_item.Type): $($cmd_item.Command)"
        Add-Finding "WARNING" "PowerShell History" "$($cmd_item.Type) - $($cmd_item.Command)"
    }
} else {
    Write-Host "  ✓ No suspicious PowerShell commands" -ForegroundColor Green
}
...
# [24/29] REGISTRY CHEAT CONFIGURATION CHECK
Write-Host "[24/29] Scanning registry for cheat configurations..." -ForegroundColor Yellow
Write-Log "[24/29] REGISTRY CHEAT CONFIG DETECTION"

$reg_cheats = Get-Registry-Cheat-Config

if ($reg_cheats.Count -gt 0) {
    Write-Host "🚨 CHEAT CONFIGURATIONS FOUND IN REGISTRY!" -ForegroundColor Red
    
    foreach ($config in $reg_cheats) {
        Write-Host "  🚨 Registry Key: $($config.RegistryKey)" -ForegroundColor Red
        Write-Host "    Path: $($config.Path)" -ForegroundColor Red
        Write-Host "    Cheat: $($config.Cheat)" -ForegroundColor Red
        Write-Host "    Last Modified: $($config.LastModifiedFormatted)" -ForegroundColor Yellow
        Write-Host "    Age: $($config.DaysOld) days old" -ForegroundColor Yellow
        Write-Log "  🚨 Registry: $($config.RegistryKey) (Cheat: $($config.Cheat))"
        Write-Log "    Last Modified: $($config.LastModifiedFormatted)"
        Write-Log "    Days Old: $($config.DaysOld)"
        Add-Finding "CRITICAL" "Registry Cheat Config" "$($config.RegistryKey) - $($config.Cheat) (Modified: $($config.LastModifiedFormatted))"
    }
} else {
    Write-Host "  ✓ No cheat configs in registry" -ForegroundColor Green
}
Write-Log ""

# [25/29] LAUNCHER PROFILES & MOD LOADER ANALYSIS
Write-Host "[25/29] Analyzing launcher profiles and mod loaders..." -ForegroundColor Yellow
Write-Log "[25/29] LAUNCHER PROFILE ANALYSIS"

$launcher_info = Get-Launcher-Profile-Analysis

Write-Host "  Mod Loaders:" -ForegroundColor Cyan
if ($launcher_info.HasFabric) {
    Write-Host "    • Fabric - ✓ Installed" -ForegroundColor Green
    Write-Log "  ✓ Fabric loader detected"
}
if ($launcher_info.HasForge) {
    Write-Host "    • Forge - ✓ Installed" -ForegroundColor Green
    Write-Log "  ✓ Forge loader detected"
}

if ($launcher_info.Profiles.Count -gt 0) {
    Write-Host "  Game Profiles: $($launcher_info.Profiles.Count)" -ForegroundColor Cyan
    Write-Log "  Total profiles: $($launcher_info.Profiles.Count)"
    
    foreach ($profile in $launcher_info.Profiles) {
        if ($profile.Suspicious) {
            Write-Host "    🚨 $($profile.Name) - SUSPICIOUS" -ForegroundColor Red
            Write-Log "  🚨 Suspicious profile: $($profile.Name)"
            Add-Finding "WARNING" "Suspicious Profile" "$($profile.Name) - Unusual JVM args"
        }
    }
}

if ($launcher_info.Issues.Count -gt 0) {
    Write-Host "  ⚠ Issues found:" -ForegroundColor Yellow
    foreach ($issue in $launcher_info.Issues) {
...
# [26/29] Generating final verdict...
Write-Host "[26/29] Generating final verdict..." -ForegroundColor Yellow
Write-Log ""
Write-Log "═════════════════════════════════════════════════════════"
